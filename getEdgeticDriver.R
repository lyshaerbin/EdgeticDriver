...
#' Driver.mut=getEdgeticDriver(Dysnet,Mut,alpha=0.05,n.sim=1000)
#'@export
getEdgeticDriver <- function(Dysnet,Mut,alpha,n.sim){
  ...
  N11=length(unique(Both.dys$Sam.ID))#######the number of samples with dysregulated edges
  N22=length(unique(Mut$Tumor_Sample_Barcode))###########the number of samples with mutation
  FF.net=c()
  for(ii in 1:dim(Mutdys)[1]){
    print(ii)
    xx=which(Both.dys$GeneA==Mutdys$GeneA[ii]&Both.dys$GeneB==Mutdys$GeneB[ii])
    Dys.sam=unique(Both.dys$Sam.ID[xx])
    A=Three.sam(Dys.sam)
    Xmut1=which(Mut$Hugo_Symbol==Mutdys$GeneA[ii])
    Xmut2=which(Mut$Hugo_Symbol==Mutdys$GeneB[ii])
    Xmut=union(Xmut1,Xmut2)
    Gene.mut=Mut[Xmut,c(1,5:7,9,12,13)]
    Gene.mut=unique(Gene.mut)
